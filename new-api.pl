...
$mux->add($listener, Buffered => new IO::Buffered::Split(qr/\n/));
$mux->add($listener, Buffered => new IO::Buffered::Size(["n", 0]);
$mux->add($listener, Buffered => new IO::Buffered::FixedSize(100);
$mux->add($listener, Buffered => new IO::Buffered::Regexp(qr/(.*?)\n/);
$mux->add($listener, Buffered => new IO::Buffered::Last();
$mux->add($listener, Buffered => new IO::Buffered::HTTP();

$mux->add($listener, Buffered => new IO::Buffered(Split => qr/\n/));
$mux->add($listener, Buffered => new IO::Buffered(Size => ["na*", 0]));
$mux->add($listener, Buffered => new IO::Buffered(FixedSize => 100));
$mux->add($listener, Buffered => new IO::Buffered(Regexp => qr/(.*?)\n/));
$mux->add($listener, Buffered => new IO::Buffered(Last => 1));
$mux->add($listener, Buffered => new IO::Buffered(HTTP => 1));
